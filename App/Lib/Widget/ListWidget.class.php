...
class ListWidget extends Widget {
	...
	public function render( $data ){
		...
		if( !empty( $data['alias']) ) {//用别名指定输出分类文章列表
			$category = $categoryDao->where( array('alias'=>$data['alias'], 'hardware' => 'pc','lang'=>$lang) )->find();
			if(!$category){
		...
		if( $data['is_top'] ) {//是否置顶再显示,可以显示从第N条显示到第N条
			$where['is_top'] = 1;
		}
		...
		if( $category['tpl_one']=='list' || $category['tpl_one']=='auto' ){
			...
			if($data['model'] == 'advert'){
				$newsList = $newsDao->where ( $where )->order('ordernum desc, create_time desc')->limit ( $page->firstRow . ',' . $page->listRows )->select ();
			}else{
				$newsList = $newsDao->where ( $where )->order('is_top desc, ordernum desc, create_time desc,id desc')->limit ( $page->firstRow . ',' . $page->listRows )->select ();
			}
			//echo $newsDao->getLastSql();
			//	dump($newsList);exit;
			...
			$emptyInfo = L('no_msg');
			if($isOne == false){
			return $this->renderFile($data['style'], array('dataList'=>$newsList, 'location'=>$location, 'emptyInfo'=>$emptyInfo,'category'=>$category, 'module'=>MODULE_NAME, 'cid'=>$cid,'data'=>$data, 'pageBar'=>$page->show()),$data['tplNum'],$data['rowpage']);
			}else{
				return '';
			}
		} else {
